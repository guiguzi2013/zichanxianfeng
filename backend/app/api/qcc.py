...
- 缓存：同一企业 24h 内重复查询直接返回缓存（零调用、零积分），所有用户共享
- 流程：工商 12 工具 + 35 维风险扫描 + 有记录维度明细
"""
import asyncio
import json
import logging
...
QCC_TOKEN = _QCC_SETTINGS.qcc_token or "Mau7RHYTJ5L8OYlz9xM3Yn6a7EVN7lqBkjHf2NB21DyqxcxI"
QCC_HOST = "agent.qcc.com"
CACHE_TTL = timedelta(hours=24)
# 负缓存：工商登记+风险扫描都失败的名称视为"查无此名"，1h 内不再重复调用（防积分浪费）
...
async def query_full(company: str) -> dict:
    ...
    async with httpx.AsyncClient() as client:
        ...
        for tool, label in COMPANY_TOOLS:
            r = await company_mcp.call(client, tool, {"searchKey": company})
            biz[tool] = {"label": label, **r}

        scan = await risk_mcp.call(client, "get_company_risk_scan", {"searchKey": company})

        details: dict = {}
        if scan.get("ok") and isinstance(scan.get("data"), dict):
            for f in scan["data"].get("风险因子扫描") or []:
                if (f.get("条目数") or 0) > 0 and f.get("明细工具"):
                    r = await risk_mcp.call(client, f["明细工具"], {"searchKey": company})
                    details[f["明细工具"]] = {
                        "label": f["风险因子"],
                        "factor": f["风险因子"],
                        "count": f["条目数"],
...
# 财产线索页展示的工具（轻量模式，8 次调用/企业）
# 注意：动产抵押/土地抵押/司法拍卖属于风险路由，必须用 risk_mcp 调用
PROPERTY_TOOLS = [
    ("get_company_registration_info", "工商登记"),
    ("get_external_investments", "对外投资"),
    ("get_shareholder_info", "股东信息"),
...
async def query_property_clues(company: str) -> dict:
    ...
    async with httpx.AsyncClient() as client:
        ...
        biz: dict = {}
        for tool, label in PROPERTY_TOOLS:
            r = await company_mcp.call(client, tool, {"searchKey": company})
            biz[tool] = {"label": label, **r}
        ...
        for tool, label in RISK_PROPERTY_TOOLS:
            r = await risk_mcp.call(client, tool, {"searchKey": company})
            biz[tool] = {"label": label, **r}

    result = {"company": company, "biz": biz, "risk": {"scan": scan, "details": {}}}
    # 失败不缓存（防止 24h 缓存投毒）；工商登记或风险扫描任一成功即视为可缓存
...
# 深度关注的司法/资产因子（有记录才拉明细；动产/土地抵押、司法拍卖已在 clues 基底中）
...
def deep_credit_estimate(scan_data: dict | None) -> int:
    """深度调查积分预估：基底 8（未缓存时）+ 公司增量 7 + 有记录因子明细数"""
    extra = 0
    if scan_data and isinstance(scan_data, dict):
        for f in scan_data.get("风险因子扫描") or []:
            if (f.get("条目数") or 0) > 0 and f.get("明细工具") in DEEP_RISK_FACTOR_TOOLS:
                extra += 1
    return 8 + len(DEEP_COMPANY_TOOLS) + extra


async def query_deep_investigation(company: str) -> dict:
    """深度调查：财产线索基底（clues 缓存 → plain 全量缓存 → 实查）+ 增量工商 + 有记录司法因子明细

    基底优先级：clues: 键（含 3 个风险路由财产工具）> plain 全量键（缺 3 个财产工具，需补查）> 实查。
    """
    cached = cache_get(f"deep:{company}")
    if cached:
        return cached
    # 1. 基底：clues 缓存优先；plain 全量缓存兜底（结构兼容，但缺动产/土地抵押、司法拍卖 3 个风险路由工具）
    base = cache_get(f"clues:{company}") or cache_get(company)
    async with httpx.AsyncClient() as client:
        company_mcp = McpClient("/mcp/company/stream")
        risk_mcp = McpClient("/mcp/risk/stream")
        await asyncio.gather(company_mcp.init(client), risk_mcp.init(client))
        ...
        if base is None:
            # 无任何缓存：先走轻量财产查询（8 调用），失败会写负缓存
            base = await query_property_clues(company)
        else:
            # plain 全量缓存缺 3 个风险路由财产工具 → 补查，避免报告缺维度
            missing = [t for t, _ in RISK_PROPERTY_TOOLS if t not in (base.get("biz") or {})]
            if missing:
                for tool, label in RISK_PROPERTY_TOOLS:
                    if tool in missing:
                        r = await risk_mcp.call(client, tool, {"searchKey": company})
                        base.setdefault("biz", {})[tool] = {"label": label, **r}

        extra_biz: dict = {}
        for tool, label in DEEP_COMPANY_TOOLS:
            r = await company_mcp.call(client, tool, {"searchKey": company})
        ...
        if scan.get("ok") and isinstance(scan.get("data"), dict):
            for f in scan["data"].get("风险因子扫描") or []:
                ...
                if (f.get("条目数") or 0) > 0 and t in DEEP_RISK_FACTOR_TOOLS:
                    r = await risk_mcp.call(client, t, {"searchKey": company})
                    details[t] = {"label": f["风险因子"], "count": f["条目数"], **r}

    result = {
        "company": company,
        "base": base,               # 财产线索基底（工商/投资/股东/抵押/拍卖/扫描）
        "extra_biz": extra_biz,     # 增量工商
        "risk_details": details,    # 深度司法因子明细
    }
    reg_ok = (base.get("biz", {}).get("get_company_registration_info") or {}).get("ok")
    if reg_ok:
...
async def query_engine_summary(company: str) -> dict:
    """尽调引擎节点②用：工商登记 + 股东 + 风险扫描 + 高危因子明细（比 query_full 轻，省积分）"""
    ...
    async with httpx.AsyncClient() as client:
        ...
        reg = await company_mcp.call(client, "get_company_registration_info", {"searchKey": company})
        shr = await company_mcp.call(client, "get_shareholder_info", {"searchKey": company})
        scan = await risk_mcp.call(client, "get_company_risk_scan", {"searchKey": company})

        details: dict = {}
        if scan.get("ok") and isinstance(scan.get("data"), dict):
            for f in scan["data"].get("风险因子扫描") or []:
                if (f.get("条目数") or 0) > 0 and f.get("明细工具") in ENGINE_RISK_TOOLS:
                    r = await risk_mcp.call(client, f["明细工具"], {"searchKey": company})
                    details[f["明细工具"]] = {
                        "label": f["风险因子"],
                        "count": f["条目数"],
...
class QccQueryRequest(BaseModel):
    company: str
    mode: str = "full"  # full=全量(企查查演示页) / clues=财产线索(轻量，省积分)
...
async def qcc_query(payload: QccQueryRequest):
