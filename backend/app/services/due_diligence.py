...
async def _ai_judicial_note(claim: Claim, er: dict) -> str | None:
    ...
    try:
        system = (
            ...
            "表达风格（2026-09-08 用户确认）：资深律师执业建议口吻，结合给定数据差异化表述，避免空泛套话。 每条先给一句话结论再展开；只讲与本债权相关的要点，不罗列无关法条/判例/情形，同一要点不重复论证；风险提示与应对成对出现；用词精炼不堆砌。\n"
            "必须严格按以下 JSON 格式输出（只输出 JSON，不要多余文字）：{\"note\": \"你的解读文字\"}"
        )
        user = json.dumps({
...
async def _ai_disposal_note(claim: Claim, disposal: dict, nodes: dict) -> str | None:
    ...
    try:
        system = (
            ...
            "表达风格（2026-09-08 用户确认）：资深律师执业建议口吻，结合该债权的具体对象与司法状态差异化表述，避免空泛套话。\n"
            "必须严格按以下 JSON 格式输出（只输出 JSON，不要多余文字）：{\"note\": \"你的解读文字\"}"
        )
        paths_brief = [
